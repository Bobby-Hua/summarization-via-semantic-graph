...
from amr_model_gtos.generator import Generator
...
class LEDEncoderSelfAttention(nn.Module):
    ...
    def _compute_global_attn_output_from_hidden(
        ...
        if graph_hidden_states is not None:
            seq_len=graph_hidden_states.shape[0]
            graph_hidden_states=graph_hidden_states.unsqueeze(1)
            is_index_masked=torch.zeros((1,seq_len),dtype=torch.bool, device=graph_hidden_states.device)

            global_query_vectors_only_global = self.query_global(global_attn_hidden_states)
            global_key_vectors = self.key_global(graph_hidden_states)
            global_value_vectors = self.value_global(graph_hidden_states)
...
class LEDDecoderLayer(nn.Module):
    def __init__(self, config: LEDConfig):
        ...
        if self.config.graph_late_fusion:
            self.amr_attn = LEDDecoderAttention(
                self.embed_dim,
                config.decoder_attention_heads,
                dropout=config.attention_dropout,
                is_decoder=True,
            ...
            self.amr_linear = nn.Linear(512, self.embed_dim)
    ...
    def forward(
        ...
        if graph_hidden_states is not None: #late fusion
            if encoder_hidden_states.size()[0]!=1:
                graph_hidden_states = graph_hidden_states.expand(encoder_hidden_states.size()[0],-1,-1)

            residual = hidden_states

            cross_attn_past_key_value = past_key_value[4:6] if past_key_value is not None else None
            hidden_states, cross_attn_weights, cross_attn_present_key_value = self.amr_attn(
                hidden_states=hidden_states,
            ...
            hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
            hidden_states = residual + self.resweight*hidden_states
...
class LEDEncoderBaseModelOutput(ModelOutput):
    ...
    graph_out: Optional[Tuple[torch.FloatTensor]] = None
    graph_masks: Optional[Tuple[torch.FloatTensor]] = None
    node_selection_logits: Optional[Tuple[torch.FloatTensor]] = None
    node_selection_tgt: Optional[Tuple[torch.FloatTensor]] = None
...
class LEDEncoder(LEDPreTrainedModel):
    ...
    def __init__(self, config: LEDConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.graph_cross_attn = config.graph_cross_attn
        self.graph_late_fusion = config.graph_late_fusion
        if config.graph_cross_attn or config.graph_late_fusion:
            amr_configs = copy.deepcopy(config.amr_configs) #the original config must remain serializable for ckpt pointing
            from amr_model.data_gsp import Vocab, LexicalMap, STR, END, CLS, SEL, TL, rCLS
            amr_configs['vocabs']['concept'] = Vocab(amr_configs['vocabs']['concept'], 5, [CLS])
            amr_configs['vocabs']['token'] = Vocab(amr_configs['vocabs']['token'], 5, [STR, END])
            amr_configs['vocabs']['predictable_token'] = Vocab(amr_configs['vocabs']['predictable_token'], 5, [END])
            amr_configs['vocabs']['token_char'] = Vocab(amr_configs['vocabs']['token_char'], 100, [STR, END])
            amr_configs['vocabs']['concept_char'] = Vocab(amr_configs['vocabs']['concept_char'] , 100, [STR, END])
            amr_configs['vocabs']['relation'] = Vocab( amr_configs['vocabs']['relation'], 5, [CLS, rCLS, SEL, TL])
            amr_configs['device'] = torch.device(amr_configs['device'])
            if config.graph_cross_attn:
                self.graph_linear = nn.Linear(config.amr_configs['embed_dim'], config.hidden_size)
        if config.graph_cross_attn:
            self.segment_embed = nn.Embedding(config.max_seg_num, config.d_model)

            self.amr_encoder = Generator(**amr_configs)
            if config.node_selection_loss:
                self.node_selection_layer = nn.Linear(512, 256)
                self.node_dropout = nn.Dropout(p=0.1)
                self.node_selection_out = nn.Linear(256,1)
    ...
    def _get_scene_sent_indices(self,input_ids, bos_token_id, utter_token_id=None):

        is_bos = (input_ids == bos_token_id)
        is_bou = (input_ids == utter_token_id)
        global_attn_indices = torch.logical_or(is_bos, is_bou)
        seg_num=torch.cumsum(is_bos, dim=1)
        bos_pos = is_bos.nonzero()

        return global_attn_indices, seg_num
    ...
    def forward(
        ...
        Args:
            ...
            return_dict (`bool`, *optional*):
                Whether or not to return a [`~utils.ModelOutput`] instead of a plain tuple.
        """
        if self.graph_late_fusion or self.graph_cross_attn:
            graph_output=self.amr_encoder(amr_features)
            graph_hidden_states = graph_output[0].transpose(0,1)
            graph_masks = graph_output[1].transpose(0,1)

          
            indices_to_keep = ~graph_masks
            indices_to_keep = indices_to_keep.nonzero(as_tuple=True)
            graph_hidden_states = graph_hidden_states[indices_to_keep]

         
            node_selection_logits = None
            node_selection_tgt = None

            graph_hidden_states = self.graph_linear(graph_hidden_states)
            seg_ids = torch.arange(graph_masks.size(0),device=graph_hidden_states.device,dtype=input_ids.dtype)
            seg_ids = seg_ids.unsqueeze(-1).expand(graph_masks.size())
            seg_ids=seg_ids[indices_to_keep]
            seg_embed=self.segment_embed(seg_ids)
            graph_hidden_states+=seg_embed
            _, unique_idx=np.unique(seg_ids.cpu().numpy(), return_index=True)
        ...
        elif self.graph_cross_attn:
         
            global_attention_mask, tmp_seg=self._get_scene_sent_indices(input_ids, self.config.bos_token_id, self.config.utter_token_id)
            attention_mask = self._merge_to_attention_mask(attention_mask, global_attention_mask)

            utter_locs=global_attention_mask.nonzero(as_tuple=True)
            seg_ids_utter=tmp_seg[utter_locs]
           
            inputs_embeds[utter_locs]+=seg_embed[unique_idx[seg_ids_utter.cpu().numpy()-1]]
        ...
        if attention_mask is not None:
            ...
            expanded=_expand_mask(attention_mask, inputs_embeds.dtype)
            attention_mask = expanded[:, 0, 0, :]
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                if self.gradient_checkpointing and self.training:
                    ...
                    layer_outputs = torch.utils.checkpoint.checkpoint(
                        ...
                        graph_hidden_states if graph_hidden_states is not None else None,
                        graph_masks if graph_masks is not None else None,
                    )
                else:
        ...
        if not self.graph_late_fusion:
            del graph_hidden_states
            graph_hidden_states=None

        if not return_dict:
            return tuple(
                v for v in [hidden_states, encoder_states, all_attentions, all_global_attentions, graph_hidden_states, graph_masks, node_selection_logits, node_selection_tgt] if v is not None
            )
        return LEDEncoderBaseModelOutput(
            ...
            graph_out=graph_hidden_states,
            graph_masks=graph_masks,
            node_selection_logits=node_selection_logits,
            node_selection_tgt=node_selection_tgt,
        )
...
class LEDModel(LEDPreTrainedModel):
    ...
    def forward(
        ...
        decoder_outputs = self.decoder(
            ...
            graph_hidden_states=encoder_outputs.graph_out,
        ...
        return LEDSeq2SeqModelOutput(
            ...
            node_selection_logits=encoder_outputs.node_selection_logits,
            node_selection_tgt=encoder_outputs.node_selection_tgt,
        )
...
class LEDForConditionalGeneration(LEDPreTrainedModel):
    ...
    def forward(
        ...
        return LEDSeq2SeqLMOutput(
            ...
            node_selection_logits=outputs.node_selection_logits,
            node_selection_tgt=outputs.node_selection_tgt,
        )
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        return {
            ...
            "amr_features": kwargs['amr_features'],
        }
