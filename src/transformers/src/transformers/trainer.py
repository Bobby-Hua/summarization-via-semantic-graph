...
class Trainer:
    ...
    def __init__(
        ...
        if self.model.config.node_selection_loss:
            self.node_selection_bce=nn.BCEWithLogitsLoss(pos_weight=torch.tensor(self.model.config.node_selection_pos_weight))
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    with model.no_sync():
                        tr_loss_step = self.training_step(model, inputs, step=step)
                else:
                    tr_loss_step = self.training_step(model, inputs, step=step)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], step: int) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            loss = self.compute_loss(model, inputs, step=step)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, step=None):
        ...
        if outputs.node_selection_tgt is not None:
            node_selection_loss=self.node_selection_bce(outputs.node_selection_logits,outputs.node_selection_tgt)*self.model.config.node_selection_loss_weight
            self.writer.add_scalar('node_selection_loss', node_selection_loss, global_step=self.state.global_step)
            pred=torch.sigmoid(outputs.node_selection_logits).round().cpu().detach().numpy()
            tgt=outputs.node_selection_tgt.cpu().detach().numpy()
            acc=(pred == tgt).mean()
            f1=f1_score(tgt,pred,average='macro')
            self.writer.add_scalar('node_selection_acc', acc, global_step=self.state.global_step)
            self.writer.add_scalar('f1', f1, global_step=self.state.global_step)
            # print((tgt==0.).sum()/tgt.sum())
            loss+=node_selection_loss
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        # pydevd_pycharm.settrace('160.39.213.176', port=7777, stdoutToServer=True, stderrToServer=True)
        logger.info(f"Saving model checkpoint to {output_dir}")
        # Save a trained model and configuration using `save_pretrained()`.
        # They can then be reloaded using `from_pretrained()`
        if not isinstance(self.model, PreTrainedModel):
