...
from amr_model_gtos.encoder import TokenEncoder, RelationEncoder
from amr_model_gtos.decoder import DecodeLayer
from amr_model_gtos.customized_transformer import Transformer, SinusoidalPositionalEmbedding, SelfAttentionMask
from amr_model_gtos.graph_transformer import GraphTransformer
from amr_model_gtos.data import ListsToTensor, ListsofStringToTensor, STR
from amr_model_gtos.search import Hypothesis, Beam, search_by_batch
...
class Generator(nn.Module):
    def __init__(self, vocabs, 
                ...
                pretrained_file, device, rel_num_heads=None, max_m=None, shared_token_embed=None):
        ...
        self.token_encoder = TokenEncoder(vocabs['token'], vocabs['token_char'],
                        word_char_dim, word_dim, embed_dim, cnn_filters, char2word_dim, dropout, pretrained_file)
    ...
    def encode_step(self, inp, train=True):
        ...
        if train:
            relation = relation.index_select(0, inp['relation'].reshape(-1)).reshape(*inp['relation'].size(), -1)
        else:
            #pick = inp['relation'][:,:,:,0]
    ...
    def forward(self, data):
        ...
        return concept_repr, concept_mask, data['cp_seq']
